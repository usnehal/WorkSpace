...
def train(cfg) -> None:
    ...
    print("training: experiment %s" % (cfg["exp_name"]))
    ...
    print("started tensorboard writer")

    model = CAE()
    model.train()
    logger.info(f"loaded model on cfg['device']")
    print(f"loaded model on cfg['device']")
    ...
    print(f"loaded dataset from {cfg['dataset_path']}")
